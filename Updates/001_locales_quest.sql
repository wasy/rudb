...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =3361;
...
-- UPDATE `locales_quest` SET `progress` = '' WHERE `entry` =24469;
...
-- UPDATE `locales_quest` SET `progress` = '' WHERE `entry` =24470;
...
-- UPDATE `locales_quest` SET `progress` = 'Удалось раздобыть что-нибудь у троггов?' WHERE `entry` =24486;
...
-- UPDATE `locales_quest` SET `progress` = '' WHERE `entry` =24471;
UPDATE `locales_quest` SET `OfferRewardText_loc8` = 'Многие из спасенных тобою уже вернулись в лагерь. Раны отлично перебинтованы! Из тебя может получиться хороший лекарь, если такая карьера тебя привлекает.' WHERE `entry` =24471;

-- UPDATE `locales_quest` SET `progress` = 'Народ начинает нервничать. Ты принес припасы?' WHERE `entry` =24487;
UPDATE `locales_quest` SET `OfferRewardText_loc8` = 'Добро пожаловать, |3-6($C). $gГотов:Готова взяться за работу?' WHERE `entry` =24487;

-- UPDATE `locales_quest` SET `progress` = '' WHERE `entry` =24473;
...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =24489;
...
-- UPDATE `locales_quest` SET `progress` = 'Пиво, дорогуша, пиво!' WHERE `entry` =24474;
...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =24490;
...
-- UPDATE `locales_quest` SET `progress` = 'Народ начинает нервничать. Ты принес припасы?' WHERE `entry` =24475;
...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =24491;
...
-- UPDATE `locales_quest` SET `progress` = 'Ты точно уверен, что разобрался со всеми делами? Повернуть назад во время полета не выйдет.' WHERE `entry` =24492;
...
-- UPDATE `locales_quest` SET `progress` = 'Да?' WHERE `entry` =24477;
...
-- UPDATE `locales_quest` SET `progress` = 'Ты точно уверен, что разобрался со всеми делами? Повернуть назад во время полета не выйдет.' WHERE `entry` =24493;
...
-- UPDATE `locales_quest` SET `progress` = 'Духи предупредили меня о твоем приближении, $N. А ты их слышал?' WHERE `entry` =24494;
...
-- UPDATE `locales_quest` SET `progress` = 'Ты еще не освоил \"Стихийный удар\". Попросишь вежливо – я тебя ему научу.' WHERE `entry` =24527;
...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =218;
...
-- UPDATE `locales_quest` SET `progress` = 'Как только я получу свои инструменты, ноги моей тут больше не будет.' WHERE `entry` =182;
